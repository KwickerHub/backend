				...
				if(num_count_user_project($connect, $project_name, $user_id) > 0){
					...
					if($project_content != ""){
						...
						if(fopen($project_path, "w")){
							...
							if( file_exists("../../storage/projects/") ){
								// we can move on
							}else{
								// lets create them now
								$array = ["status"=>"false", "user"=>$user_existance, "msg"=>"incomplete creation. Please Rename project and retry."];
								mkdir("../../storage/projects", 0777, true);
								chmod("../../storage/projects", 0777);
							}
						}
					}
				}else{
